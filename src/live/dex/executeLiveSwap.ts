...
export async function executeLiveSwap(
	...
	const slippageMultiplier = BigInt(10_000 - input.maxSlippageBps);
	const minExpectedBuy =
		(legs.expectedBuyAmount * slippageMultiplier) / 10_000n;
	if (
		!passesSlippageCheck({
			expectedBuyAmount: minExpectedBuy,
