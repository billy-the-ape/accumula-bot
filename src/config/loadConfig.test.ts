...
describe("loadConfig", () => {
	...
	it("loads CDP paymaster config without optional gas policy id", () => {
		const config = loadConfig({
			...validEnv,
		...
		expect(config.live.cdpGasPolicyId).toBeUndefined();
		expect(config.live.cdpGasPaymentMode).toBe("sponsor");
	});

	it("loads optional CDP gas policy id when provided", () => {
		const config = loadConfig({
			...validEnv,
			CDP_PAYMASTER_RPC_URL:
				"https://api.developer.coinbase.com/rpc/v1/base/test-token",
